    ...
    this.httpClientCache = newBuilder().build(key -> {
      ...
      return new HttpClient() {
        ...
        public HttpResponse send(HttpRequest request, int responseTimeout, boolean followRedirects,
          ...
          return innerClient.send(request, responseTimeout, followRedirects, authentication);
        }

        @Override
        public CompletableFuture<HttpResponse> sendAsync(HttpRequest request, int responseTimeout, boolean followRedirects,
                                                         HttpAuthentication authentication) {
          return innerClient.sendAsync(request, responseTimeout, followRedirects, authentication);
        }
      };
    });
  }
